# -*- coding: utf-8 -*-
"""
Created on Sun Jul 15 12:47:05 2018
...
class RNN_config(object):
    ...
    def __init__(self, num_steps, state_size=4):
...
def setup_graph(graph, config):
    with graph.as_default():
        ...
        ## change tf.split(1, config.num_steps, x_one_hot) to tf.split(x_one_hot, config.num_steps, 1)
        rnn_inputs = [tf.squeeze(i, squeeze_dims=[1]) for i in tf.split(x_one_hot, config.num_steps, 1)]
        
        # Turn our y placeholder into a list of one-hot tensor
        y_one_hot = tf.one_hot(y, config.num_classes)
        ## change tf.split(1, config.num_steps, y_one_hot) to tf.split(y_one_hot, config.num_steps, 1)
        y_as_list = [tf.squeeze(i, squeeze_dims=[1]) for i in tf.split(y_one_hot, config.num_steps, 1)]
        ...
        def rnn_cell(rnn_input, state):
            ...
            ## change tf.concat(1, [rnn_input, state]) to tf.concat([rnn_input, state], 1)
